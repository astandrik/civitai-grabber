import os
import requests
from tkinter import Tk, Label, Entry, Button, StringVar, Text, Scrollbar, messagebox, ttk
...
MAX_WORKERS = 10  # Number of parallel threads for downloading
...
def download_images_with_prompts(api_key, model_id):
    headers = {
    ...
    while url:
        ...
        if response.status_code != 200:
            log_message(f"Error: Failed to retrieve data: {response.status_code}")
    ...
    log_message(f"Downloading {total_images} images and saving prompts...")
    download_images_parallel(image_data_list, total_images)
    messagebox.showinfo("Success", "All images and prompts downloaded successfully!")

def download_images_parallel(image_data_list, total_images):
    completed = 0
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for future in as_completed(futures):
            try:
                ...
                messagebox.showerror("Error", f"Error downloading image or saving prompt: {e}")
...
def start_download():
    ...
    if not api_key or not model_id:
        messagebox.showerror("Error", "Please enter both API Key and Model ID!")
        return
    ...
    download_thread = threading.Thread(target=download_images_with_prompts, args=(api_key, model_id))
...
Entry(root, textvariable=api_key_var, width=40).grid(row=0, column=1, padx=10, pady=10)

# Model ID input
Label(root, text="Model ID:").grid(row=1, column=0, padx=10, pady=10)
model_id_var = StringVar()
Entry(root, textvariable=model_id_var, width=40).grid(row=1, column=1, padx=10, pady=10)

# Download Button
Button(root, text="Download Images", command=start_download).grid(row=2, columnspan=2, pady=20)
